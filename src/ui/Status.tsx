...
export function Status({ status }: { status: TransactionStatus }) {
  switch (status) {
    case 'SUCCESS':
      return (
        <div className="bg-success rounded-big text-green flex items-center gap-x-1 justify-self-start py-[5px] pr-2 pl-2.5 text-xs font-medium">
          <div className="bg-green mt-0.5 h-1.5 w-1.5 rounded-full"></div>
          <span>Success</span>
        </div>
      );
    case 'PROCESSING':
